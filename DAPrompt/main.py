...
args.model = args.model + 'model_planB1.40_' + 'fold-' + str(args.fold) + '__' + t + '.pth'
...
for epoch in range(args.num_epoch):
    ...
    for i, batch_indices in enumerate(all_indices, 1):
        ...
        batch_arg, mask_arg, label, label1, label2, clabel, mask_indices = get_batch(train_data, batch_indices)
        ...
        predt=[]
        # answer_space1：[50270,50267]  answer_space2：[50270,50266]
        ...
        if i % (3000 // args.batch_size) == 0:
            printlog('loss={:.4f}, acc={:.4f}, Recall={:.4f} F1_score={:.4f}'.format(
